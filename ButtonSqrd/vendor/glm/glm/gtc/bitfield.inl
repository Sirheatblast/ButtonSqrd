...
#	pragma clang diagnostic pop
	...
	template<typename genIUType>
	GLM_FUNC_QUALIFIER genIUType bitfieldFill one(genIUType Value, int FirstBit, int BitCount)
	{
		return Value | static_cast<genIUType>(mask(BitCount) << FirstBit);
	}

	template<length_t L, typename T, qualifier Q>
	GLM_FUNC_QUALIFIER vec<L, T, Q> bitfieldFill one(vec<L, T, Q> const& Value, int FirstBit, int BitCount)
	{
