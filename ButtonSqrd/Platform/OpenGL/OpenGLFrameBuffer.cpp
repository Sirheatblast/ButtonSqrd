...
namespace BtnSqd {
    ...
    void BtnSqd::OpenGLFrameBuffer::AttatchTextureArray(TextureType texType, unsigned int arraySize) {
        ...
        if (GetTextureArrayType(texType) == GL_TEXTURE_CUBE_MAP_ARRAY) {
            glTexImage3D(GetTextureArrayType(texType), 0, GL_DEPTH_COMPONENT, width, height, 6* arraySize, 0, GL_DEPTH_COMPONENT, GL_FLOAT, nullptr);
            glTexStorage3D(GL_TEXTURE_CUBE_MAP_ARRAY, 0, GL_DEPTH_COMPONENT, width, height, 6 * arraySize);
        }
        else {
            glTexImage3D(GetTextureArrayType(texType), 0, GL_DEPTH_COMPONENT, width, height, arraySize, 0, GL_DEPTH_COMPONENT, GL_FLOAT, nullptr);
        }
    ...
    void BtnSqd::OpenGLFrameBuffer::AttatchTextureArray(TextureType texType, unsigned int arraySize, unsigned int nWidth,unsigned int nHeight) {
        ...
        if (GetTextureArrayType(texType) == GL_TEXTURE_CUBE_MAP_ARRAY) {
            glTexImage3D(GetTextureArrayType(texType), 0, GL_DEPTH_COMPONENT, nWidth, nHeight, 6 * arraySize, 0, GL_DEPTH_COMPONENT, GL_FLOAT, nullptr);
            glTexStorage3D(GL_TEXTURE_CUBE_MAP_ARRAY, 0, GL_DEPTH_COMPONENT, nWidth, nHeight, 6 * arraySize);
        }
        else {
            glTexImage3D(GetTextureArrayType(texType), 0, GL_DEPTH_COMPONENT, nWidth, nHeight, arraySize, 0, GL_DEPTH_COMPONENT, GL_FLOAT, nullptr);
        }
