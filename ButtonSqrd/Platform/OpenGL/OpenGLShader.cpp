...
namespace BtnSqd {
	...
	void OpenGLShader::SetFloatArray(const std::string& name, std::vector<float> value) const {
		GLint location = glGetUniformLocation(renderId, name.c_str());
	...
	void OpenGLShader::SetMat4Array(const std::string& name, std::vector<glm::mat4> value) const	{
		glUseProgram(renderId);
		glUniformMatrix4fv(glGetUniformLocation(renderId, name.c_str()), static_cast<GLsizei>(value.size()), GL_FALSE, glm::value_ptr(value[0]));
		glUseProgram(0);
	}
}