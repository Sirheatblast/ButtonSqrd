	...
	WindowsWindow::WindowsWindow(int width, int height, const char* name, bool isLocked) {
		...
		glfwSetMouseButtonCallback(window, [](GLFWwindow* window, int button, int action, int mods) {
			...
			case GLFW_PRESS: {
				...
				double dTime = cTime - data.lastMouseClickTime[button];
				if (dTime > 0.0 && dTime < 0.25) {
					data.callback(new OnMouseDoubleClickEvent(button));
					data.lastMouseClickTime[button] = 0.0;
				}
				else {
					data.lastMouseClickTime[button] = cTime;
				}
